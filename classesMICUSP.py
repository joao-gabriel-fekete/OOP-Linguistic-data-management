...
sample = "taggedfilesMICUSP\BIO.G0.01.1_F_NS.txt"
micuspData = pd.read_csv("micusp_papers.csv")
...
class TaggedFile:
    ...
    def mainInfo(self):
        ...
        self.fileID = re.findall(r"(?<=\\)[A-Z].*\d{1}(?=\_)", self.path)[0]

    def getRegister(self):
        lookup = micuspData['PAPER ID'].isin([self.fileID]).any()
        if lookup == True:
            registerRow = micuspData.loc[micuspData['PAPER ID']==self.fileID]
        self.register = registerRow.iloc[0]['PAPER TYPE']
